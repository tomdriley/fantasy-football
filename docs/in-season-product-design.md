...
Status: implemented and validated, September 6, 2026.
...
This redesign changes presentation and workflow, not the underlying strategy or
its unproven competitive advantage.
...
3. Next lineup deadline, with affected players.
4. A short "What needs attention" summary:
   - lineup changes or unavailable starters;
   - injury-tag checks without instructing the user to bench every questionable player;
   - incomplete data or blocked roster edits.
5. Recommended lineup, with ordinary position labels and snapshot-time projections.
6. Optional pickups in a collapsed section, with one-week gain and drop/waiver caveats.
...
4. On success, show the baseline's next actions and target lineup.
5. The user makes any changes in Sleeper. This app does not submit them.
...
| Recent valid advice | Show specific attention items and deadline | Baseline only, still subject to current news and app verification |
...
| Questionable player | "Check availability before kickoff" | Not an automatic bench instruction |
...
| Streaming recommendation | Optional pickup |
