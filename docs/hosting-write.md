...
**Deployed September 13, 2026; ready for the approved user's first button test.**
The operator confirmed the earlier Google login → approval → sample → logout
checkpoint and authorized this isolated synthetic write. The existing single
enrolled identity is unchanged and remains private. Cloud preparation,
configuration, readiness and credential-free denials are verified; **the actual
Google-authenticated POST, duplicate/reload and subsequent logout checks remain
pending**. No account marker was seeded.

[Open the protected write test](https://thomasriley-fantasy-w3-pilot-stage.azurewebsites.net/fantasy-football/test-write).
If signed out, use the existing hosting page's Google sign-in, then return to
this link. Verify no marker exists, click **Record test write**, and repeat/reload
to confirm the original timestamp. Finally sign out and verify access is denied.
Do not share session identity output, cookies or tokens.

## Verified deployment record

- Target: only `thomasriley-fantasy-w3-pilot/stage`, `ff-westus3-pilot`, WestUS3.
- Runtime source release: `f8e622620d9c4dea44b4a8fdfa100e65ec12feac`.
- Reviewed immutable public **hosting-only** image:
  `ghcr.io/tomdriley/fantasy-football-hosting@sha256:11702550be21b80c9374e9f20c2e2766dbea32f5689a1216997893f7c3c89987`.
- Implementation `af5ed5a7f2886348a01ae619594a7032f989ea77` merged through
  [private PR #4](https://github.com/tomdriley/fantasy-football/pull/4), without
  forced pushes or guard bypass. The source repository remains private.

| Gate | Successful workflow / evidence |
| --- | --- |
| Private branch candidate | [34770187221](https://github.com/tomdriley/fantasy-football/actions/runs/34770187221): all 129 tests passed, including real PostgreSQL 17; Bicep compilation and non-root container checks passed |
| Required PR check | [34770197702](https://github.com/tomdriley/fantasy-football/actions/runs/34770197702): passed before merge |
| Exact-main private candidate | [34770564599](https://github.com/tomdriley/fantasy-football/actions/runs/34770564599): all 129 tests passed; complete template/container build |
| Separate reviewed publication | [34770681596](https://github.com/tomdriley/fantasy-football/actions/runs/34770681596): all 129 tests passed; exact main release above |
| Same-digest stage deployment | [34770947940](https://github.com/tomdriley/fantasy-football/actions/runs/34770947940): successful anonymous pull, both capability/lock checks, Azure OIDC, pre/post auth validation and hosted verification |

Scoped source, build-context and private operator security reviews reported no
high-confidence vulnerabilities. The image audit checked **all ten layers**,
matched twelve explicitly allowed `/app` files to source, and confirmed the four
base layers match the earlier reviewed image. All layer bytes, including tar
metadata, were checked for the private enrolled subject and forbidden admin
markers without publishing them. Candidate/rebuild inventories and every
non-bytecode file matched; 443 generated bytecode files were semantically
equivalent. Anonymous retrieval of the published manifest, configuration and
every layer matched the audited artifact. SQL migrations, private operator
scripts/settings, tests, docs, Git history and advisor/Yahoo data are excluded.
This is not a full-repository disclosure review or exhaustive dependency audit.

The private ACI job used the retained reviewed authentication image, its
non-root UID 10001 (also asserted in-process), private DNS and TLS `verify-full`.
Admin, reader and new writer credentials were injected only through ARM Key
Vault secure references. The new password was generated in memory and stored
only as `pg-write-probe-password`; no admin or Google secret was read locally.
Azure rejected an explicit ACI `securityContext` property before any job ran;
the supported retry retained the audited image's non-root `USER` and verified
UID, without adding a NAT gateway or opening public database access.

Database verification established:

- Identical before/after reader role, settings, memberships, existing object
  grants and exact three synthetic sample rows.
- Dedicated writer role and schema, forced RLS, runtime privilege/catalog checks,
  **12 permission/RLS denials**, and rolled-back synthetic insert/idempotence
  checks. No persistent test transaction or user-derived owner was inserted.
- **Zero persisted markers** at preparation completion; no account marker was
  created by readiness, deployment or credential-free checks.
- Private PostgreSQL and seven-day PITR retained. The completed ACI group,
  privileged job deployment and ephemeral parameter files were removed.

Activation set the persistent write lock before enabling the phase, retained the
auth lock/client, stopped only stage, and reviewed a two-resource what-if:
stage app settings plus the new secret-scoped grant. Exact read-back changed
only `FFOPT_HOSTING_PHASE` and six `FFOPT_WRITE_DB_*` fields. The actual enrolled
allowlist, Google configuration, reader settings, existing grants, parent/sticky
settings and blog/article configurations were unchanged. All three Key Vault
references resolved; the only new grant is Key Vault Secrets User for the
existing stage identity on **only** the writer secret. The reviewed digest was
selected while stopped, then stage was explicitly started and redeployed by
the real OIDC workflow above.

Final checks passed for health, catalog-only writer readiness and the exact
release/phase. **22** anonymous/forged GET/HEAD and fully shaped POST probes
returned 401; **60** other method probes returned the expected 405 on public
paths or 401 on protected/unknown paths. The blog remained HTTP 200 and the
disabled fantasy parent HTTP 403. The existing Node browser-script fixture
passed; a loopback-only real-browser transport attempt could load the page,
but browser-control tools failed, so real-browser Origin behavior is **not**
claimed verified here. No real Google browser session was operated.

Safe receipts and private snapshots remain outside Git in the session's
`files/write-probe-rollout/`; no identities, secret values or authenticated
response bodies appear in this ledger. The remaining gate is the approved
user's positive button → duplicate/reload → logout check, followed by a private
row-count/unchanged-reader check. Do not mark that gate complete from mocks or
anonymous 401 responses.

The retained authentication-only rollback release is
`d471de3e65084373a4b3ea9984c108029e72681f`, digest
`sha256:3728be4b5d5951d575a040861d45d509b715bf4d3f60b889906b7c392f6565dc`.
It is eligible only after the stopped-slot writer-setting removal procedure
below; never run it with writer credentials still configured.
...
The initial rollout completed steps 1–7 below; step 8 remains a user action.
These are also the guarded procedure for future explicitly approved changes,
not permission to repeat provisioning or replace existing enrollment.
...
Initial local implementation verification: **129 tests discovered, 110 passed, 19 real
...
available then. The completed private-CI, real PostgreSQL, container/layer and
cloud gates are recorded above; CI ran all 129 tests without fixture skips.
